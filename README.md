...
Notes are available through the Omeka API at `/api/side_notes`, so scripts and
external tools can read and write them. The API must be enabled in
**Settings → API**, and the caller needs a key belonging to a user with an
admin role — notes are staff-only and anonymous callers are refused.

Creating or updating a note is a single call. Because a record can hold only
one note, `POST` replaces an existing note rather than failing, so the caller
never has to check first:

```
POST /api/side_notes?key=YOUR_KEY
{"record_type": "Item", "record_id": 1991, "note": "text here"}
```

Reading the note attached to a record:

```
GET /api/side_notes?record_type=Item&record_id=1991&key=YOUR_KEY
```

`record_type` is `Item` or `Collection`. `GET` by id, `PUT` and `DELETE` are
also supported. Overwriting preserves the original `created` and
...
See [CHANGELOG.md](CHANGELOG.md). Latest release: **2.3.0**.
